...
void executeCommand(const String& commandId, int relayDurationMs) {
  ...
  if (!reported) {
    ...
    // Note: the command is already executed — the relay pulsed correctly.
    // The backend will time out and investigate if no result arrives.
  }
...
int httpPost(const String& path, const String& body, String& responseOut) {
  ...
  // ⚠️  For production: load the server's CA cert instead of skipping verification.
  // For MVP development, setInsecure() is acceptable on a private network.
  // Replace with client.setCACert(root_ca) before going live.
  client.setInsecure();
...
int httpGet(const String& path, String& responseOut) {
  ...
  HTTPClient http;
  WiFiClientSecure client;
  client.setInsecure();  // See note above
